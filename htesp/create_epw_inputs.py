...
# FIX(3): `read` was used in ciftoxsf() but never imported -> NameError on
# every `ciftoxsf` condition.
from ase.io import read
from htesp.kpoint_path import kpoint_path
from htesp.wannier90 import epw_bandcheck
from htesp.check_json import config


def _is_gamma(qpoint, tol=1e-8):
    """
    True when the three q-point components are all zero.

    Parameters
    ----------
    qpoint : sequence of str or float
    ...
    """
    try:
        return all(abs(float(component)) < tol for component in qpoint[:3])
...
def phonon_input(mass_file, qpoint_file, mpid, compound, prefix):
    ...
    with open("ph-{}-{}.in".format(mpid,compound), 'w') as ph_write:
        ...
        if not os.path.isfile("ph-q.in"):
            ...
            # FIX(5): epsil used to be written for *any* non-'T' entry, at any
            # q and for a magnetic system alike.  docs/otherinput.rst documents
            # it as "non magnetic and at q = 0", and QE rejects epsil for a
            # metal, for q != 0 and for a spin-polarised run.
            is_magnetic = bool(config().get('pwscf_in', {}).get('magnetic', False))
            at_gamma = _is_gamma(nqpoint)
            if is_metal in ('T', 't'):
                pass
            elif not at_gamma:
                print("epsil not set: q = {} {} {} is not the zone centre\n".format(
                    nqpoint[0], nqpoint[1], nqpoint[2]))
            elif is_magnetic:
                print("epsil not set: pwscf_in.magnetic is true "
                      "(epsil is only valid for a non-magnetic system)\n")
            else:
                # Non metal, non magnetic, q = 0 0 0 -> turn epsil on.
...
def prepare_nscf(grid_file,what='nscf'):
    ...
    # FIX: os.system("kmesh_nscf.py ... > out") replaced by subprocess.run with
    # an argument list and an explicit stdout file - no shell, no redirection.
    argv = [sys.executable, "-m", "htesp.kmesh_nscf",
            str(int(grid[0])), str(int(grid[1])), str(int(grid[2]))]
    if what == 'wan':
        argv.append("wan")
        out_file = "wann_grid.out"
    else:
        out_file = "nscf_grid.out"
    with open(out_file, "w") as grid_out:
        subprocess.run(argv, stdout=grid_out, check=True)
...
def band_file_epw(mpid,compound):
    ...
    src = "scf_dir/scf-{}-{}-band.in".format(mpid, compound)
    dst = "scf_dir/{}-{}-band.in".format(mpid, compound)
    if os.path.isfile(src):
        # FIX: `os.system("sed '/&SYSTEM/a nosym = .true.,' src > dst")`
        # interpolated compound names into a shell string; done in Python now.
        with open(src, "r") as read_band:
            lines = read_band.readlines()
        with open(dst, "w") as write_band:
            for line in lines:
                write_band.write(line)
                if "&SYSTEM" in line:
                    write_band.write("  nosym = .true.,\n")
    else:
        # Print an error message if the file is not found
        print("scf-{}-{}-band.in not found inside scf_dir.\n".format(mpid, compound))
        print("Run the script 'mainprogram.py process', with process from 1 to 4\n")
...
def pw2wan_input(mpid,compound,prefix,proj=" "):
    ...
    if proj == "scdm":
        ...
        if os.path.isfile("scdm_dir/scdm-{}-{}".format(mpid,compound)):
            ...
            mu_scdm = data[0]
            sigma_scdm = data[1]
    # FIX: isdir-then-shell-"mkdir" replaced by makedirs(exist_ok=True).
...
def epw_sc_from_json(json_file, out="epw.in"):
    ...
    # imported htesp.check_json.config inside this function.
    with open(json_file, 'r') as f:
        epw_config = json.load(f)
    # Write EPW input file
    with open(out, "a") as epw_write:
        for key, value in epw_config.items():
            if isinstance(value, bool):
                ...
                for i,_ in enumerate(epw_config['wdata']):
...
def epw_sc(mpid,compound,prefix,qpoint_file,kpoint_file,proj=" ",out="epw.in"):
    ...
    with open(out, "w") as epw_write:
        ...
        # FIX(4): these three writes used to sit *after* the `with` block, so
        # they ran against a closed file and raised
        # "ValueError: I/O operation on closed file" - the whole `epw`
        # condition was dead.  They are now inside the block, and the JSON
        # settings are appended to `out` (not to a hard-coded 'epw.in') once
        # this handle is closed.
        epw_write.write("\n")
        epw_write.write("#From epw.json file\n")
    # Write input from epw.json file (opens `out` in append mode itself)
    epw_sc_from_json("epw.json", out=out)
...
def main():
    ...
    if condition == "ph":
        ...
        # FIX: os.system("echo 3 3 3 > kmesh.grid") replaced by a file write.
        if not os.path.isfile("kmesh.grid"):
            with open("kmesh.grid", "w") as write_grid:
                write_grid.write("3 3 3\n")
        ...
        pwd = os.getcwd()
        matches = glob.glob(pwd+"/**/cif/{}.cif".format(mpid), recursive=True)
        if not matches:
            raise FileNotFoundError(
                "no cif/{}.cif found under {}".format(mpid, pwd))
        file_path = matches[0]
        print(file_path)
        ...
        # FIX: `file_name` used to stay unbound on the miss, so the call below
        # raised NameError instead of reporting the missing file.
        ...
        if dft in ('QE', 'qe'):
            ...
            # FIX: os.system("mv ...") replaced by shutil.move.
            os.makedirs("epw_dir", exist_ok=True)
            shutil.move("ex.win", "epw_dir/ex-{}-{}.win".format(mpid, compound))
        else:
