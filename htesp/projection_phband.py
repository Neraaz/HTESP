...
def reading_input(filename):
    ...
    for i in range(na_t):
        ...
        ions[name] = indices
        nat += len(indices)
    # FIX(24): the two header fields used to be re-parsed inside a loop over
    # *every* line of the file, so whether the "#" comment was stripped was
    # decided by the LAST line rather than by lines 0 and 1 themselves.  Each
    # header line is now stripped of its own comment.
