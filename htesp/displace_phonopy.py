...
class DisplacePhonopy:
    ...
    def applydisplace(self):
        ...
        if self.eigen is None:
            raise ValueError("call geteigenvec(iqpt, imode) before applydisplace()")
        if self.mass is None:
            raise ValueError("call mass_extract() before applydisplace()")
        struc = structure.Structure.from_file("POSCAR")
        for i,site in enumerate(struc.sites):
            # FIX(20): self.eigen[i] is a nested list straight out of YAML
            # ([[re, im], [re, im], [re, im]]); dividing a list by a float
            # raised "TypeError: unsupported operand type(s) for /".
            displace_eigen = np.array(self.eigen[i], dtype=float) / np.sqrt(self.mass[i])
