...
def plot_phase(data):
    ...
    for i in range(data.shape[0]):
        if os.path.isfile("R{}-{}/relax/POSCAR".format(data.ID[i],data.comp[i])):
            ...
            # ``struc`` would otherwise be unbound (or stale from the previous
            # iteration) and the entry silently wrong.
    ...
    with open("convexhull.csv", "w") as write_convexhull:
        # FIX(16): the header named six columns but every row wrote five --
        # the 'decomposition' value is produced by the
        # phase_diag.get_decomposition() call that is commented out below, so
        # pandas read the file with a shifted/NaN last column.  The header now
        # names exactly the five fields the rows write; re-enable the
        # commented-out write below *and* this column together if it is wanted.
        write_convexhull.write("ID,total energy (eV),comp,FE(eV/atom),e_above_hull_calc\n")
...
def main():
    ...
    which is the name ``htesp.workflow`` writes (header
    ``ID,comp,NIONS,energy,niteration``) and the name the legacy
    ``legacy/bash/phonopy-scan`` wrote -- FIX(17): verified, no mismatch.
