...
class PROCARProcessor:
    ...
    def clean(self):
        ...
        """
        # FIX: os.system(f"rm {file_}") interpolated an element name into a
        # shell string; os.remove does the same job without a shell.
        for elm in self.elements:
            for file_ in glob.glob(f"{elm}-*.dat.gnu"):
                try:
                    os.remove(file_)
                except OSError as err:
                    print(f"could not remove {file_}: {err}\n")
        ...
        with open(f"{orb}.dat.gnu", "w") as write_procar:
            ...
            for i in range(self.nband):
                ...
                if self.nspin == 2:
                    ...
                    procar_iband = self.data[:, i, self.nion, 1:self.ncol]
                    procar_iband = procar_iband[:, find_all_indices(self.col,orb)]
                # FIX(1): the nspin == 2 assignment was commented out, so
                # `procar_jband` was simply undefined -> NameError on every
                # spin-polarised band projection.  data[spin, kpt, band, ion,
                # col] reduces to shape (2, nkpoint) once the first matching
                # orbital column is taken - exactly what the [0, j] / [1, j]
                # indexing below expects, and the same "[0]" the nspin == 1
                # branch takes.
                if self.nspin == 2:
                    procar_jband = procar_iband[:, :, 0]
                # Loop over k-points
                for j in range(self.nkpoint):
                    if self.nspin == 2:
    ...
    def write_band_element(self, elm):
        ...
        with open(f"{elm}.dat.gnu", "w") as write_procar:
            for i in range(self.nband):
                ...
                # FIX(1): same undefined `procar_jband` as in
                # write_band_orbital.  After the sum over the element's ions
                # procar_iband already has shape (2, nkpoint), which is what
                # the [0, j] / [1, j] indexing below reads.
                if self.nspin == 2:
                    procar_jband = procar_iband
                for j in range(self.nkpoint):
                    if self.nspin == 2:
                        write_procar.write(str(j+1) + " " + str(procar_jband[0,j]) + " " +str(procar_jband[1,j]) + "\n")
                    else:
...
def _is_logical_triplet(line):
    """True when `line` holds exactly three Fortran logicals (T/F)."""
    fields = line.split()
    return len(fields) == 3 and all(
        field.strip(".").upper() in ("T", "F", "TRUE", "FALSE")
        for field in fields)


def parse_proj_sizes(lines, legacy_index=12):
    """
    Locate the ``natomwfc nkstot nbnd`` line of a projwfc.x output.

    FIX(24): the caller used to read ``lines[12]`` outright.  That position
    depends on the number of species and atoms in the header, so it broke for
    anything but the cell the code was written against - and it silently gave
    the wrong numbers rather than failing.
    ...
    legacy_index : int
        Line used as a last resort (the old hard-coded 12).

    Returns
    -------
    tuple
        ``(natomwfc, nkstot, nbnd, lsda)`` - the first three as ints, `lsda`
        as a bool (True for a spin-polarised run).
    """
    for i, line in enumerate(lines[:-1]):
        ...
        if len(fields) != 3:
            continue
        try:
            natomwfc, nkstot, nbnd = (int(field) for field in fields)
        except ValueError:
            continue
        if _is_logical_triplet(lines[i + 1]):
            lsda = lines[i + 1].split()[0].strip(".").upper().startswith("T")
            return natomwfc, nkstot, nbnd, lsda
    # Last resort: the historical fixed position.
    print("WARNING: could not locate the 'natomwfc nkstot nbnd' line by shape; "
          f"falling back to line {legacy_index}\n")
    natomwfc, nkstot, nbnd = (int(field) for field in lines[legacy_index].split())
    return natomwfc, nkstot, nbnd, False
...
class DataProcessor:
    ...
    def clean(self,delete_file):
        """
        Delete a file produced during processing.

        Args:
            delete_file (str): Path of the file to remove.
        """
        # FIX: os.system(f"rm {delete_file}") interpolated a path into a shell
        # string; os.remove does the same job with no shell involved.
        try:
            os.remove(delete_file)
        except OSError as err:
            print(f"could not remove {delete_file}: {err}\n")
    ...
    def _parse_proj_file(self, structure_file, proj_file):
        ...
        # Read the projection file once and keep it.
        with open(f"{projfile}", "r") as read_projfile:
            self.projdata = read_projfile.readlines()
        # Extract orbital information for each element from the projection file
        # FIX: os.system(f"grep {elm} {projfile} | sed '1d' > ...") interpolated
        # an element symbol into a shell string; the same selection (every
        # matching line except the first) is done in Python.
        for elm in element:
            matches = [line for line in self.projdata if elm in line]
            with open(f"proj-{elm}.dat", "w") as write_elm:
                for line in matches[1:]:
                    write_elm.write(line)
        # FIX(24): `self.projdata[12]` hard-coded the position of the
        # "natomwfc nkstot nbnd" line, which in a projwfc.x header sits after a
        # variable number of species/atom lines and moves with nat/ntyp.  The
        # line is located by its shape instead: three integers followed by the
        # three T/F flags (lsda, noncolin, lspinorb).
        natomwfc, self.nkpoint, self.nband, self.lsda = parse_proj_sizes(self.projdata)
    ...
    def combine_orbital(self, orb, key=None, if_gnu=False):
        """
        Combine projection data for one orbital and save it to a file.

        FIX(2): this method used to be defined twice with different
        signatures - ``combine_orbital(self, key, orb, if_gnu=False)`` earlier
        in the class (unreachable, shadowed by the later definition) and
        ``combine_orbital(self, orb)`` here.  The two are merged: `key=None`
        keeps the behaviour of the definition that actually ran (sum the same
        orbital across *all* elements into ``<orb>.dat``), while passing `key`
        restores the element-scoped variant (sum the sub-orbitals of one
        element into ``<key>-<orb>.dat``).  The only caller,
        ``dp.combine_orbital(orb)`` in main(), is unchanged.

        Args:
            orb (str): Orbital type for which projection data is combined.
            key (str, optional): Element to scope the combination to.
                None (default) combines across every element.
            if_gnu (bool): Only used with `key` - also write a .gnu file for
                each per-orbital file before combining. Default False.

        Returns:
            str: Name of the combined .dat file that was written.
        """
        if key is None:
            ...
            if not files:
                raise FileNotFoundError(
                    f"no '*-{orb}.dat' projection files to combine")
            data = None
            for i, file_ in enumerate(files):
                self.ref_data = file_
                if i == 0:
                    ...
                    data[:, 2] += newdata[:, 2]
            out = f"{orb}.dat"
            np.savetxt(out, data, fmt=['%d', '%d', '%.4f'])
            self.convert_gnu(out)
            return out
        # Element-scoped variant (the formerly unreachable definition).
        self.write_proj_elm_orbi()
        ...
        if not sfile:
            raise FileNotFoundError(
                f"no '{key}-*{orb}*.dat' projection files to combine")
        sdata = np.zeros((self.nrow, 3))
        s0 = np.loadtxt(sfile[0])
        sdata[:, 0] = s0[:, 0]
        sdata[:, 1] = s0[:, 1]
        for name in sfile:
            idata = np.loadtxt(name)
            sdata[:, 2] += idata[:, 2]
        out = f"{key}-{orb}.dat"
        np.savetxt(out, sdata, fmt=['%d', '%d', '%.4f'])
        self.convert_gnu(out)
        return out
    ...
    if dft in ("VASP", "vasp"):
        ...
        if proj_type == "element":
            ...
            for elm in proj:
                ...
                for projection in proj_list:
                    # FIX: os.system("rm ...") -> dp.clean() (os.remove).
                    dp.clean(f"{elm}-{projection}.dat.gnu")
        elif proj_type == "orbital":
            ...
            for elm_orb in proj:
                elm = elm_orb
                orb = proj[elm]
                # FIX: bare `except:` replaced by the exception a missing
                # orbital name actually raises when it is not a key of
                # final_dict[elm] (KeyError), plus the file/parse errors the
                # suborbital fallback is meant to cover.
                if isinstance(orb,list):
                    for orbi in orb:
                        try:
                            dp.extract_data(elm,orbi)
                        except (KeyError, OSError, ValueError, IndexError) as err:
                            print(f"{elm}-{orbi}: {type(err).__name__}: {err}; "
                                  "retrying as a combined orbital\n")
                            dp.extract_data_2(elm,orbi)
                else:
                    try:
                        dp.extract_data(elm,orb)
                    except (KeyError, OSError, ValueError, IndexError) as err:
                        print(f"{elm}-{orb}: {type(err).__name__}: {err}; "
                              "retrying as a combined orbital\n")
    ...
    if dft in ("VASP", "vasp"):
        ...
        for _,proj_file in enumerate(proj_files):
            if proj_file not in f"{comp}.dat.gnu":
                ...
                # FIX: os.system("mv ...") -> shutil.move.
                shutil.move(f"{comp}-band.pdf", f"{comp}-{proj_name}-band.pdf")
    elif dft in ("QE", "qe"):
        proj_files = glob.glob("*.dat.gnu")
        print("-----------------------------------\n")
        print(f"Band file: {comp}.dat.gnu")
